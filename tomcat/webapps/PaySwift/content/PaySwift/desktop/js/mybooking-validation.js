...
PaymentValidation = {
	...
	validate: function(frm, beforepost, onsuccess, onerror, validateOnly) {
		...
		for(var i=0;i<allElements.length;i++) {
			...
			switch(eType) {
				case "radio":
					if($("input:radio[name="+field_name+"]:checked").length==0) {
						is_error = true;
					};
				break;
				case "checkbox":
				...
				for(var index=0;index<ruleTypeArr.length;index++) {
					...
					case "supportedCard" : 
							...
							var PaymentOption = ruleSplit[2];
		 					...
		 					cards [5] = {cardName: "Dinners", payOption : "cc,dc,emi"};
					...
					case "checkReward" :
							...
							var totAmount = paymentJSON.amount;
		...
		function showErrorMessage(field_name, error_holder, msg) {
			...
			Toast.show(errormsg, field_name, undefined, function() {}, isMobile);
		}
	}
};
