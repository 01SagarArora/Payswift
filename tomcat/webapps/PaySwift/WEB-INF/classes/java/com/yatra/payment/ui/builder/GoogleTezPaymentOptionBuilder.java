...
import com.yatra.payment.ui.display.beans.GoogleTezPaymentOption;
...
@Service
public class GoogleTezPaymentOptionBuilder implements PaymentOptionBuilder {

	private static final Logger logger = Logger.getLogger(GoogleTezPaymentOptionBuilder.class);
	...
	public PaymentOption build(PaymentOption paymentOption) throws Exception {
		try {
			GoogleTezPaymentOption googleTezPaymentOption = new GoogleTezPaymentOption(paymentOption);
	...
	public PaymentOption build(String merchantCode, String productCode) throws Exception {
		...
		logger.info("Getting Google Tez Payment Option bean for merchant : " + merchantCode + " and product : " + productCode);

		// Check if OxigenB2B payment option is enabled for given merchant and product. 
		Boolean isPayOpEnabled = statusProviderService.isPaymentOptionEnabled(merchantCode, productCode, PaymentUIUtil.PAYMENT_OPTION_GOOGLE_TEZ);
		...
		if(isPayOpEnabled == null) {
			logger.info("Could not be determined if Google Tez enabled for merchant : " + merchantCode + " and product : " + productCode + ". Returning null..");
			return null;
		}
		// Get OxigenB2B PaymentOption bean which is common for all merchants and products.
		PaymentOption commonPaymentOption = paymentOptionCache.get(PaymentUIUtil.PAYMENT_OPTION_GOOGLE_TEZ);
		...
		GoogleTezPaymentOption googleTezPaymentOption = new GoogleTezPaymentOption(commonPaymentOption);
		googleTezPaymentOption.setStatus(PaymentUIUtil.getStatus(isPayOpEnabled));
		logger.info("Returning Google Tez Payment Option bean for merchant : " + merchantCode + " and product : " + productCode);
