...
@Service
public class QuickbookServiceImpl implements MiniServiceI {
	...
	public JSONObject getRequiredData(JSONObject requestJson, JSONObject responseJSON) {
		...
		try{
			...
			if(isCrpProduct && getSSOId(requestJson).compareTo(0L) == 0){
				...
				quickBookPaymentOption = quickBookUIService.getQuickBookBean(requestJson.getString("merchant"), requestJson.getString("product"), requestJson.getString("ssoToken"), getSSOId(requestJson), getBookingType(requestJson));
			}
		} catch (Exception e) {
