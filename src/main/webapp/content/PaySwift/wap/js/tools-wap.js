...
var Toaster = function(msg, w, timeout, callbackAfter) {
		...
		if($("#"+id).length==0) {
			//create new one
			$(target).parent().after("<span id='"+id+"' class='ui-icon-label ui-icon-error input-error mobileToasterHolder' style='clear:both;'></span>");
		}
		$("#"+id).html(sMsg).hide();
...
//		console.debug(" left "+left + " top "+top);
		...
		if(!Toaster.isInit) {
			Toaster.isInit = true;
			$("body").append("<style>.toasterHolder{background:#333333; padding:10px; text-align:center; float:left;color:#fff; font-size:11px; letter-spacing:1px; -moz-border-radius:5px; border-radius:5px;}.toasterHolder .toasterContent{text-align:left; color:#fff;}</style>");
		}
};
...
var MS_Loader = {
		isInit: false,
		loaderImg:'http://img.yatra.com/fresco/resources/default/images/ajax-loader-gray-white.gif',
		secureImg:'https://secure.yatra.com/fresco/resources/default/images/ajax-loader-gray-white.gif',
		...
		init: function() {
			if(this.isInit == false) {
				this.isInit = true;
				var loaderImg = (window.location.protocol=="https:") ? this.secureImg : this.loaderImg;
